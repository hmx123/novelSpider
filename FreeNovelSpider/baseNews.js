...
;(function (global, $) {
    ...
    var _util = {
        initNameSpace: function (router, root) {
            ...
            var p = root || window, arrNS = router.split('.');
            ...
            var query = str.substr(qIndex), result = query.match(new RegExp("[\?\&]" + name + "=([^\&]+)", "i"));
            ...
            var me = this, success = option.success;
            if (!('timeout' in option)) {
                option.timeout = 3000;
                if ('config' in CS && 'env' in CS.config && CS.config.env.envType !== 'release') {
...
;(function ($) {
    var _left_separator = '<%', _right_separator = '%>';
    ...
    function _compileTpl(tpl, data) {
        ...
        var lineTag = '\n', logic_tag = '\x1b', isNewEngine = ''.trim,
        ...
        for (var i = 0, len = arrTpl.length; i < len; i++) {
            var item = arrTpl[i], char0 = item.charAt(0);
            if (char0 !== logic_tag) {
                if (char0.length) {
    ...
    function _parseSyntax(code, arrOut) {
        code = code.replace(/^\s+/, '');
        var splitCode = code.split(' '), operator = splitCode.shift(), args = splitCode.join(' ');
        switch (operator) {
            case'if':
                code = 'if(' + args + '){';
                break;
            ...
            case'/if':
                ...
                break;
            case'each':
                var list = splitCode[0] || '$list', as = splitCode[1] || 'as', val = splitCode[2] || '$value',
    ...
    function bindEvent(root_id, events, scope) {
        ...
        var node = {'root': root}, attr_nodes = $(root).find("[attr]").toArray(), el = null, str_attr = '',
            arr_attr = null, t = null, arr_bind = [];
    ...
    CS.uiBinder = {'bindData': bindData, 'bindEvent': bindEvent, 'appendHTML': appendHTML};
})(jQuery);
;(function ($) {
    var _util = CS.util;
    var _defaultConfig = {'opacity': 0.5, 'isBlurBackground': true, 'zIndex': 1000000};
    var _maskCss = "maskUI", _win = $(window), _doc = $(document),
    ...
    CS.mask.prototype = {
        open: function () {
            var position = _util.isIE6() ? 'absolute' : 'fixed', zIndex = _defaultConfig.zIndex++;
            if (_util.isIE6()) {
...
;(function ($) {
    ...
    function _setEnv() {
        var currentUrl = window.location.href, releaseVersion = '0913102115', urlConfig = {
            'dev': {
            ...
            'uat': {
                ...
                'cssUrl': 'http://uat.sta1.chuangshi.qq.com/chuangshi/' + releaseVersion + '/css/merge/'
            },
            'release': {
                'siteUrl': 'http://chuangshi.qq.com/',
                'cssUrl': 'http://sta1.chuangshi.qq.com/chuangshi/' + releaseVersion + '/css/merge/'
            }
        };
...
;(function ($) {
    var _util = CS.util, _uiBinder = CS.uiBinder, _mask = CS.mask;
    var _dialogTpl = ['<div attr="inner:popup;" class="fwinmask" style="display:none;">', '<table cellpadding="0" cellspacing="0" class="fwin">', '<tbody>', '<tr>', '<td class="t_l"></td>', '<td class="t_c"></td>', '<td class="t_r"></td>', '</tr>', '<tr>', '<td class="m_l"></td>', '<td class="m_c">', '<h3 class="flb">', '<em><%=title%></em>', '<span><a attr="click:close;" href="javascript:;" class="flbc" title="å…³é—­">å…³é—­</a></span>', '</h3>', '<div class="c altw"><%==content%></div>', '<p attr="inner:buttonBox;" class="o pns" style="display:none;"></p>', '</td>', '<td class="m_r"></td>', '</tr>', '<tr>', '<td class="b_l"></td>', '<td class="b_c"></td>', '<td class="b_r"></td>', '</tr>', '</tbody>', '</table>', '</div>'].join('');
    ...
    var _contentIcon = {"error": "alert_error", "success": "alert_right", "info": "alert_info"};
    var _remainCloseTimeTips = "$time$ç§’åŽçª—å£å…³é—­", _remainLocationTimeTips = "$time$ç§’åŽé¡µé¢è·³è½¬";
        ...
        _create: function () {
            ...
            if (this.nodes && this.nodes.buttonBox && this.config.button && this.config.button.length > 0) {
                ...
                $.each(this.config.button, function (index, item) {
                    ...
                    input.className = "pn";
                    $(input).css({"padding": "0 10px", "marginLeft": "10px", "lineHeight": "21px"});
                    if (item.focus) {
                        $(input).addClass('pnc');
                    }
    ...
    _Toast.prototype = {
        init: function () {
            ...
            var left = this.left == null ? (_util.getClientWidth() - msgEntity.find('span').width()) / 2 + scrollLeft + 'px' : this.left;
            var top = this.top == null ? (_util.getClientHeight() - msgEntity.find('span').height()) / 2 + scrollTop + 'px' : this.top;
            msgEntity.css({
                position: 'fixed',
                top: top,
                'z-index': '99',
    ...
    CS.dialog = {
        "alert": function (content, callback, noticeType) {
            var title = _util.sim2tra("æ¶ˆæ¯æç¤º"),
            ...
            return new _dialog({
                "title": title,
                "content": content,
                "button": [{"value": "ç¡®å®š", "focus": true, "callback": ok}, {"value": "å–æ¶ˆ", "callback": cancel}]
            });
...
;(function ($) {
    var _util = CS.util, _uiBinder = CS.uiBinder, _mask = CS.mask;
    var _dialogTpl = ['<div attr="inner:popup;" class="readPopup tipPopup" style="display:none;">', '<a attr="click:close;" class="closePopup" href="javascript:" title="å…³é—­"></a>', '<div attr="inner:titleBox;" class="tipTitle">', '<h3><%=title%></h3>', '</div>', '<div class="tipContent"><%==content%></div>', '<center attr="inner:buttonBox;" class="tipBtnBox" style="display:none;"></center>', '</div>'].join('');
    var _dialogDefaultConfig = {
        "title": 'æ¶ˆæ¯æç¤º',
        "content": '',
        "leftMsg": '',
    ...
    _dialog.prototype = {
        _create: function () {
            ...
            if (this.nodes && this.nodes.buttonBox && this.config.button && this.config.button.length > 0) {
                var me = this, lastIndex = this.config.button.length - 1;
                $.each(this.config.button, function (index, item) {
...
;(function ($) {
    var _util = CS.util, _env = CS.config.env;
    var _params = {};

    function init(getLoginStatusAjaxUrl, sendYWLoginStatusAjaxUrl, logoutYWSsoAjaxUrl, checkTokenAjaxUrl, webSite) {
    ...
    function loadYWssoJs() {
        ...
        $.getScript(ssoJsUrl, function () {
            yw.setParams({
                "appId": 1450000220,
                "areaId": 1,
                "returnUrl": _params.sendYWLoginStatusAjaxUrl + '?refererType=NORMAL&http_referer=' + encodeURIComponent(returnUrl),
                "target": "top",
                "ticket": "1",
                "tab": "qq",
                "tabshow": tabshow
    ...
    function logout() {
        _util.request({
            url: _params.logoutYWSsoAjaxUrl, type: "GET", dataType: "json", success: function (json) {
                if (json && json.status) {
    ...
    function _getYWssoToken(data) {
        ...
        if (!data || data.code != 0 || !data.data.ticket || !data.data.ywGuid || !data.data.loginType) {
            return;
        }
    ...
    function ywlogin_onResize(width, height) {
        var $loginPopup = $('#div_ywlogin'), $loginPopupIframe = $('#ui_ywlogin');
        if ($loginPopup.length) {
            $loginPopup.css({'visibility': 'hidden', 'width': width + 'px', 'height': height + 'px'});
...
;(function ($) {
    var _util = CS.util, _env = CS.config.env, _dialog = CS.dialog, _loginPopup = CS.ssoLogin;
    var _ajaxUrls = {'addToBookshelf': 'public/addToBookshelf', 'setAutoBuy': 'booktake/setAutoBuy'};
    ...
    function _setAutoBuy(bid) {
        ...
        _util.request({
            ...
            error: function () {
                _dialog.alert('æ“ä½œå¤±è´¥ï¼Œè¯·ç¨å€™å†è¯•');
            }
        });
    }
...
;(function ($) {
    var _util = CS.util, _env = CS.config.env, _dialog = CS.readDialog, _uiBinder = CS.uiBinder,
        _loginPopup = CS.ssoLogin;
    var _params = {}, _nodes = null, _isSearchingBook = 0, _searchResultItemIndex = -1, _searchBookTimeout = null;
    var _ajaxUrls = {
    ...
    function _renderNavBar(userInfo) {
        _nodes = _uiBinder.appendHTML($('#topNav'), $('#topNavBarTpl').val(), {
            'isLogin': userInfo.isLogin === true,
            'userInfo': userInfo
        }, {
    ...
    function _bindEvent() {
        $(_nodes.bookList).on('mouseenter', 'li', function () {
            $(this).addClass('hover');
    ...
    function _bindEventBySearchInput() {
        ...
        var $searchInput = $(_nodes.searchInput), $searchResultBox = $(_nodes.searchResultBox),
        ...
        $(document).on("click", function (e) {
            if ($(_nodes.searchInputBox).is(':visible')) {
                var $target = $(e.target);
                if ($target.closest([_nodes.searchInputBox, _nodes.showSearchInputBtnBox]).length === 0) {
                    _hideSearchInput();
                    _closeSearchResultList();
                }
            }
    ...
    function _showSearchInput(e, el, nodes) {
        ...
        $(nodes.searchInputBox).show().animate({width: '230px'}, 500, function () {
    ...
    function _submitSearch(e, el, nodes) {
        var searchType = 'all', searchValue = $.trim($(nodes.searchInput).val());
        searchValue = _filterSearchWord(searchValue);
        if (searchValue.length === 0) {
            _dialog.alert("è¯·è¾“å…¥æœç´¢å†…å®¹");
            return false;
        }
        window.location.href = _env.siteUrl + _ajaxUrls.submitSearch + "?type=" + searchType + "&wd=" + searchValue;
        return false;
    }
    ...
    function _filterSearchWord(str) {
        ...
        for (var i = 0, len = str.length; i < len; i++) {
            ...
            if (/^[A-Za-z0-9\u4E00-\u9FFFï¼šï¼Œ]{1,15}$/.test(strChar)) {
                ret += strChar;
            }
        }
        return ret;
...
;(function ($) {
    var _util = CS.util, _env = CS.config.env;
    var _params = {};
    var _cmreadUrl = 'http://wap.cmread.com/r/447389415/447389723/index.htm?cm=M33W0001';

    function init(commonResourceUrl, refreshKeyUrl, requestStatLogUrl, requestStatLogData) {
    ...
    function _initUserBehaviorLog() {
        $.getScript(_params.commonResourceUrl + "/statistics/user.min.js", function () {
